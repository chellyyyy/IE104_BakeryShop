...
.then((snapshot)=>{
    ...
    cakeData.forEach((e)=>{
      ...
      `
      <li class="completed">
        <p>${e.NameOfCake}</p><button id = 'Delbtn'><a href='#popup_del' class='detect' id='${e.ID}'>x</a></button>
      </li>
      `         
    })
...
function DeleteData(){
    var IDdel = document.addEventListener('click', function(e) {
        alert( e.target.id );
    }, false);

    remove(ref(db, "TheCakes/"+ IDdel),{
    })
    .then(() =>{
