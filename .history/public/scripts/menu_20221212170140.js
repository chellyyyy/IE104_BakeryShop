...
var base
// get(child(dbref, `UserList/${uid}/Cake${e.ID}`))
//   .then((snapshot)=>{
//     var cartBase = []
//     snapshot.forEach(childSnapshot => {
...
.then((snapshot)=>{
    ...
    document.querySelectorAll('.button-2').forEach(e => {
      e.addEventListener('click' , ()=> {
          ...
          updateChange = Number(change + change)
          ...
          .then((snapshot)=>{
            ...
            cartSum.forEach((e)=>{
              document.getElementById('counter').innerText = e //Số nhỏ 
              change3 = Number(e)
              console.log(change3)
            })
          })          
...
function getSum(snapshot) {
    (snapshot)=>{
    var getSum = []; //Data của sản phẩm lấy từ database
    snapshot.forEach(childSnapshot => {
        getSum.push(childSnapshot.val());
    }); 
    } 
}
