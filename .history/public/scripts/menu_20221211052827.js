...
.then((snapshot)=>{
    ...
    cakeData.forEach((e)=>{
      ...
      `
      <li class="item">
        <div class="img"><img id="URL${e.}"src="${e.PicOfCake}" alt="${e.NameOfCake}"></div>
    ...
    document.querySelectorAll('.button-2').forEach(e => {
      e.addEventListener('click' , ()=> {
          ...
          localStorage.setItem('cart', localCount); //Chuyển số lượng từ localstorage vào biến để gán vào số nhỏ
          proNumber = localStorage.getItem('cart');
          // alert(proNumber)
          document.getElementById('counter').innerText = proNumber; //số nhỏ thay đổi
          ...
          set(ref(db, "UserList/"+ uid + "/" + "Cart/"+ "Total"),{
            Sum: proNumber,
          })
      })
