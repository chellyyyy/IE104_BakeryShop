...
function fetchData(uid){
    ...
    .then((snapshot)=>{
        if(uid==''){
            ...
            // Ufullname = document.getElementById('name_user').innerText;
            // Uaddress = document.getElementById('add_user').innerText;
            // Uphone = document.getElementById('phone_user').innerText;
...
function updateData(uid){
    update(ref(db, "UserList/"+uid+"/"),{
        address: Ufullname.value,
        fullname: Uaddress.value,
        phone: phoneChecked
    })
    .then(() =>{
        location.reload();
