// Import the functions you need from the SDKs you need
import { initializeApp } from "https://www.gstatic.com/firebasejs/9.14.0/firebase-app.js";
import { getDatabase, ref, set, child, get, update, remove } from "https://www.gstatic.com/firebasejs/9.14.0/firebase-database.js";
import {getAuth, createUserWithEmailAndPassword, signInWithEmailAndPassword, signOut } from "https://www.gstatic.com/firebasejs/9.14.0/firebase-auth.js";

// Your web app's Firebase configuration
const firebaseConfig = {
  apiKey: "AIzaSyD_g23L-wg8ve1sVjkFYV106u0Z6rs8MHY",
  authDomain: "ie104-760b6.firebaseapp.com",
  databaseURL: "https://ie104-760b6-default-rtdb.asia-southeast1.firebasedatabase.app",
  projectId: "ie104-760b6",
  storageBucket: "ie104-760b6.appspot.com",
  messagingSenderId: "954364631488",
  appId: "1:954364631488:web:c08eb53cbb2ae44aafd025"
};

// Initialize Firebase
const app = initializeApp(firebaseConfig);
const db = getDatabase();
const auth = getAuth();
const dbref = ref(db);

const uid = localStorage.getItem('uid')
var priceStr = '';
var priceNum = '';
var priceSum = '';
var search = 0;
var ID = 0;
var replaceID
var afterchange

var updatename
var updateamout
var updateurl
var updatemoney

var realkey

//FETCH CART DATA
get(child(dbref, `UserList/${uid}/`))
.then((snapshot)=>{
  var keydata = [];
  snapshot.forEach(childSnapshot => {
    keydata.push(childSnapshot.val());
  })
  
  realkey = Object.keys(keydata[1])
  // console.log(realkey)
  realkey.forEach((e)=>{
    
    // e = e.replace('Cake','')
    // console.log(e)

    get(child(dbref, `UserList/${uid}/Cart/${e}`))
    .then((snapshot)=>{
        var cakeData = []; //Data của sản phẩm lấy từ database
        snapshot.forEach(childSnapshot => {
            cakeData.push(childSnapshot.val());
        });
        // console.log(cakeData)
        if(cakeData == ''){
          document.getElementById('rightpanel').style.display = 'none';
        } else {
          document.getElementById('empty').style.display = 'none';
          // console.log(e +'adsad')
  
        
          
          document.getElementById('containerCart').innerHTML += 
          `
            <li class="main-product">
            <div class="img-product"><img src="${cakeData[3]}"></div>
            <div class="content-product">
                <p><b>${cakeData[1]}</b></p>
                <p>Số lượng:&nbsp;<span id="amout${e}">${cakeData[0]}</span></p>
            </div>
            
            <div class="tien">
                <div><span class="money">${cakeData[2]}</span></div>
                <div>
                  <h6 class="btn" id="${e}">
                    <a>Xóa<i class="fas fa-trash-can"></i></a>
                  </h6>
                </div>
            </div>
            </li>
          `
          priceStr = cakeData[2]
          priceNum = Number(priceStr.replace('đ', '').replace('.',''))
          
          priceSum = Number(priceSum + priceNum*cakeData[0])
          
        } 
        
        
        document.querySelectorAll('.btn').forEach(e => {
          
          e.addEventListener('click', ()=>{
            
            // console.log(e.id)

            document.getElementById(`amout${e.id}`).innerText -= 1;

            // ID = e.id
            // ID = ID.replace('delbtn','')
            
            // replaceID = `amout${ID}`
            // console.log(e.id)
            
            afterchange = document.getElementById(`amout${e.id}`).innerText
            get(child(dbref, `UserList/${uid}/Cart/${e.id}/`))
            .then((snapshot)=>{
              var cartData = [];
              cartData.push(snapshot.val())
              // console.log(cartData[0].Amout)
              console.log(e.id) //ID cake

              if(afterchange == 0){
                remove(ref(db, `UserList/${uid}/Cart/${e.id}/`))
                location.reload();
              } else {
                updateamout = cartData[0].Amout
                updatename = cartData[0].Name
                updatemoney = cartData[0].Price
                updateurl = cartData[0].Url
                // console.log(updateurl)
                
                set(ref(db, `UserList/${uid}/Cart/${e.id}/`),{
                  Amout: afterchange,
                  Name: updatename,
                  Price: updatemoney,
                  Url: updateurl
                })
                set(ref(db, `UserList/${uid}/Cart/${e.id}/`),{
                  Amout: afterchange,
                  Name: updatename,
                  Price: updatemoney,
                  Url: updateurl
                })
                location.reload();
              }          

              // snapshot.forEach(childSnapshot => {
              //   cartData.push(childSnapshot.val());
              //   console.log(cartData)
              // })
              
              
              
              
              
            //   cakeData.forEach((e)=>{
            //     // console.log(e)
            //   })
            })
            
          })
        })
  
  
  
  
        localStorage.setItem('noship', priceSum);
        
        document.getElementById('noship').innerText = priceSum + 'đ';
        priceSum = Number(priceSum + 10000)
        document.getElementById('withship').innerText = priceSum + 'đ';
        console.log(priceSum)
        localStorage.setItem('Total', priceSum)
  
    })
    .catch(()=>{
      console.log('')
    });

  })

  

})